...
export default function FormListRemove({
  ...
  return (
    <Modal title="Вы абсолютно уверены?" onClose={onClose}>
      <div>
        Это действие не может быть отменено. Это приведет к необратимому
        удалению
      </div>
      <div className="mt-2">
        Пожалуйста, введите <code className="text-dark">{formListTitle}</code>{" "}
        для подтверждения.
      </div>
      <div className="form-group mt-4">
        <input
          className="form-control"
          onChange={({ target }) => setConfirmingTitle(target.value.trim())}
        />
        <div className="mt-4">
          <button
